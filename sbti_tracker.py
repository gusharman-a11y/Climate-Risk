...
.stTabs [data-baseweb="tab-list"] {
    gap: 4px; background: #F1F5F9; padding: 6px; border-radius: 10px;
    margin-bottom: 16px; border: 1px solid #E2E8F0;
}
.stTabs [data-baseweb="tab"] {
    height: 44px; padding: 0 18px; font-size: 0.95rem; font-weight: 600;
    color: #334155; background: #FFFFFF; border-radius: 6px;
    border: 1px solid #E2E8F0; transition: all 0.15s ease;
}
.stTabs [data-baseweb="tab"]:hover { background: #EFF6FF; color: #1E40AF; border-color: #BFDBFE; }
.stTabs [aria-selected="true"] {
    background: #1E40AF !important; color: #FFFFFF !important;
    border-color: #1E40AF !important; box-shadow: 0 2px 6px rgba(30,64,175,0.25);
}
.stTabs [data-baseweb="tab-highlight"] { display: none; }
.stTabs [data-baseweb="tab-border"]    { display: none; }
.country-card {
    background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 14px;
    padding: 24px 20px 20px; transition: border-color 0.15s, box-shadow 0.15s;
}
.country-card:hover { border-color: #93C5FD; box-shadow: 0 4px 16px rgba(30,64,175,0.10); }
...
if st.session_state.get("page") == "australia":
    with st.sidebar.expander("🔍 Filters", expanded=False):
        st.caption("**Filters** — change any to narrow the view; defaults show all companies.")
        f_listed_w = st.multiselect(
            "Listed", ["ASX Listed", "Private & Unlisted"], default=[], key="ftr_listed",
            help="ASX Listed = traded on the ASX. Private & Unlisted = private companies and NGER reporters.",
        )
        f_sbti_w = st.multiselect("SBTi target", ["Yes", "No"], default=[], key="ftr_sbti",
            help="Yes = company has an SBTi entry. No = no SBTi engagement.")
        f_nger_w = st.multiselect("NGER reporter", ["Yes", "No"], default=[], key="ftr_nger",
            help="Yes = NGER-obligated. No = not identified as an NGER reporter.")
        f_tier_w = st.multiselect("ASRS reporting tier", asrs_tiers_present, default=[], key="ftr_tier",
            help="AASB S2 reporting tier. Tier 1: mandatory from FY2025/26.")
        search_w = st.text_input("🔍 Search by company name", key="ftr_search")
        ...
        with st.expander("More filters", expanded=False):
            f_sector_w = st.multiselect("Sector", sectors, key="ftr_sector")
            f_priority_w = st.radio(
                "Outreach priority",
            ...
            f_yrs_w = st.slider("Years to target", min_value=-10, max_value=30,
                                value=(-10, 30), step=1, key="ftr_yrs",
                                help="Negative = target year already passed.")
            f_mq_w = st.multiselect("Climate maturity (TPI MQ)", ["0", "1", "2", "3", "4", "4*"], key="ftr_mq")
            f_cp_w = st.multiselect("CP Alignment",
                ["Aligned 1.5°C", "Aligned Below 2°C", "Aligned 2°C / NDC",
                 "Not aligned", "Insufficient disclosure"], key="ftr_cp")
            f_delivery_w = st.multiselect("Delivery RAG", ["Green", "Amber", "Red", "N/A"], key="ftr_delivery")
...
def _page_header(flag: str, title: str, subtitle: str = "") -> None:
    c_back, c_title = st.columns([1.5, 10])
    with c_back:
        if st.button("← Countries", key="nav_back"):
            _go("home")
    with c_title:
...
# ─── Country profile tab renderer (shared) ───────────────────────────────────
...
<div style="background:linear-gradient(135deg,#0F172A 0%,#1E3A5F 100%);border-radius:16px;
  ...
  <div style="font-size:2.25rem;font-weight:800;color:#F8FAFC;margin-bottom:8px">
    Corporate Climate Strategy
  </div>
  <div style="font-size:1.05rem;color:#94A3B8;max-width:580px">
    BD intelligence for Australian, Philippine, and Malaysian companies —
    SBTi targets, reporting deadlines, and climate target gaps in one place.
  </div>
</div>
""", unsafe_allow_html=True)
    ...
    # World map
    _cm = [
        {"page": "australia",   "name": "Australia",   "flag": "🇦🇺", "lat": -25.0, "lon": 133.8},
        {"page": "philippines", "name": "Philippines", "flag": "🇵🇭", "lat":  12.9, "lon": 121.8},
        {"page": "malaysia",    "name": "Malaysia",    "flag": "🇲🇾", "lat":   4.2, "lon": 102.0},
    ]
    _mfig = go.Figure()
    _mfig.add_trace(go.Choropleth(
        locations=["AUS", "PHL", "MYS"], z=[1, 1, 1],
        colorscale=[[0, "#1D4ED8"], [1, "#1D4ED8"]], showscale=False,
        marker_line_color="#93C5FD", marker_line_width=1.5, hoverinfo="skip",
    ))
    _mfig.add_trace(go.Scattergeo(
        lat=[c["lat"] for c in _cm], lon=[c["lon"] for c in _cm],
        text=[f"{c['flag']} {c['name']}" for c in _cm],
        mode="markers+text",
        marker=dict(size=14, color="#FFFFFF", symbol="circle", line=dict(color="#1D4ED8", width=2)),
        textfont=dict(size=13, color="#FFFFFF", family="Inter, Helvetica, sans-serif"),
        textposition="top center",
        hovertext=[c["name"] for c in _cm], hoverinfo="text",
    ))
    _mfig.update_geos(
        projection_type="natural earth", center=dict(lat=-5, lon=120), projection_scale=2.6,
        ...
        showframe=False, bgcolor="#0F172A", showlakes=False, showrivers=False,
    )
    _mfig.update_layout(height=360, margin=dict(l=0, r=0, t=0, b=0),
                        paper_bgcolor="#0F172A", showlegend=False)

    _mev = st.plotly_chart(_mfig, use_container_width=True,
                           on_select="rerun", selection_mode="points", key="home_map")
    if _mev and hasattr(_mev, "selection") and _mev.selection.points:
        _pt = _mev.selection.points[0]
        _idx = _pt.get("point_index", _pt.get("pointIndex", None))
        if _idx is not None and 0 <= _idx < len(_cm):
            _go(_cm[_idx]["page"])
    ...
    # Quick stats
    _n_aus = len(screen)
    _n_sbti_aus = int((screen.get("SBTi", pd.Series(dtype=str)) == "Yes").sum())
    _tc_col_home = next((c for c in ("Target Classification", "Target Classification (BD)") if c in screen.columns), None)
    _n_notarget_aus = int((screen[_tc_col_home] == "No public target").sum()) if _tc_col_home else "—"

    try:
        from modules.pse import load_pse_companies as _lpse
        _pse_n = len(_lpse())
        _pse_label = f"{_pse_n:,}"
    except Exception:
        _pse_n, _pse_label = 0, "—"
    ...
    try:
        from modules.bursa import is_available as _bursa_avail, load_bursa_companies as _lbursa
        _bursa_n = len(_lbursa()) if _bursa_avail() else 0
        _bursa_label = f"{_bursa_n:,}" if _bursa_n else "—"
    except Exception:
        _bursa_n, _bursa_label = 0, "—"
  ...
  <div style="font-size:1.15rem;font-weight:700;color:#0F172A;margin-bottom:4px">Australia</div>
  <div style="font-size:0.78rem;color:#64748B;margin-bottom:14px">ASRS · SBTi · Safeguard Mechanism</div>
  <div style="display:flex;gap:20px">
    <div><div style="font-size:1.4rem;font-weight:700;color:#1E40AF">{_n_aus:,}</div><div style="font-size:0.7rem;color:#64748B">companies</div></div>
    <div><div style="font-size:1.4rem;font-weight:700;color:#16A34A">{_n_sbti_aus}</div><div style="font-size:0.7rem;color:#64748B">SBTi engaged</div></div>
    <div><div style="font-size:1.4rem;font-weight:700;color:#DC2626">{_n_notarget_aus}</div><div style="font-size:0.7rem;color:#64748B">no target</div></div>
  </div>
</div>""", unsafe_allow_html=True)
        st.markdown("<div style='height:8px'/>", unsafe_allow_html=True)
        if st.button("Explore Australia →", key="card_aus", use_container_width=True, type="primary"):
            _go("australia")

    with _c2:
        st.markdown(f"""<div class="country-card">
  <div style="font-size:2.4rem;margin-bottom:6px">🇵🇭</div>
  <div style="font-size:1.15rem;font-weight:700;color:#0F172A;margin-bottom:4px">Philippines</div>
  <div style="font-size:0.78rem;color:#64748B;margin-bottom:14px">PSE · SEC Sustainability Reporting</div>
  <div style="display:flex;gap:20px">
    <div><div style="font-size:1.4rem;font-weight:700;color:#1E40AF">{_pse_label if _pse_n else "396"}</div><div style="font-size:0.7rem;color:#64748B">PSE listed</div></div>
    <div><div style="font-size:1.4rem;font-weight:700;color:#64748B">2</div><div style="font-size:0.7rem;color:#64748B">boards</div></div>
  </div>
</div>""", unsafe_allow_html=True)
        st.markdown("<div style='height:8px'/>", unsafe_allow_html=True)
        if st.button("Explore Philippines →", key="card_phl", use_container_width=True, type="primary"):
            _go("philippines")

    with _c3:
        st.markdown(f"""<div class="country-card">
  <div style="font-size:2.4rem;margin-bottom:6px">🇲🇾</div>
  <div style="font-size:1.15rem;font-weight:700;color:#0F172A;margin-bottom:4px">Malaysia</div>
  <div style="font-size:0.78rem;color:#64748B;margin-bottom:14px">Bursa Malaysia · Sustainability Reporting</div>
  <div style="display:flex;gap:20px">
    <div><div style="font-size:1.4rem;font-weight:700;color:#1E40AF">{_bursa_label if _bursa_n else "1,057"}</div><div style="font-size:0.7rem;color:#64748B">Bursa listed</div></div>
    <div><div style="font-size:1.4rem;font-weight:700;color:#64748B">3</div><div style="font-size:0.7rem;color:#64748B">markets</div></div>
  </div>
</div>""", unsafe_allow_html=True)
        st.markdown("<div style='height:8px'/>", unsafe_allow_html=True)
        if st.button("Explore Malaysia →", key="card_mly", use_container_width=True, type="primary"):
            _go("malaysia")


# ─── Australia page ──────────────────────────────────────────────────────────

def _render_australia() -> None:
    _page_header("🇦🇺", "Australia",
                 "ASRS mandatory reporting · SBTi targets · Safeguard Mechanism")

    # Read filter state (set by the sidebar widgets rendered above)
    f_listed   = st.session_state.get("ftr_listed", [])
    f_sbti     = st.session_state.get("ftr_sbti", [])
    f_nger     = st.session_state.get("ftr_nger", [])
    f_tier     = st.session_state.get("ftr_tier", [])
    search     = st.session_state.get("ftr_search", "")
    f_sector   = st.session_state.get("ftr_sector", [])
    f_priority = st.session_state.get("ftr_priority", "All")
    f_yrs      = st.session_state.get("ftr_yrs", (-10, 30))
    f_mq       = st.session_state.get("ftr_mq", [])
    f_cp       = st.session_state.get("ftr_cp", [])
    f_delivery = st.session_state.get("ftr_delivery", [])
...
def _render_philippines() -> None:
    _page_header("🇵🇭", "Philippines",
                 "PSE-listed companies · SBTi overlay · BD priority scoring")
...
def _render_malaysia() -> None:
    _page_header("🇲🇾", "Malaysia",
                 "Bursa-listed companies · SBTi overlay · BD priority scoring")
...
if _page == "home":
    _render_home()
elif _page == "australia":
    _render_australia()
elif _page == "philippines":
    _render_philippines()
elif _page == "malaysia":
    _render_malaysia()
